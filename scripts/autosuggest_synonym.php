...
$sql = sprintf ('SELECT `keyword`, `num_keyword`, `gram_1_word_group`, `gram_2_endings`, `gram_3_additional` FROM `%s`',
	$table);
$oop->Setnames();
$oop->query($sql);
$cc=0;
...
$cc++;
if ($returned[1]!=0) {
$aUsers[$cc]=$returned[0].' ('.$returned[1].')';
} else {
$aUsers[$cc]=$returned[0];
}
$aInfo[$cc]=$returned[2].' '.$returned[3].' '.$returned[4];
endwhile;
$oop->freeResult();
$oop->_mySQL; 
